...
namespace prog1 {
    ...
    int findMaxSumRow(Matrix *matrix) {
        ...
        for (int i = 0; i < matrix->N; i++) {
            if (matrix->rows[i]) {
                if (sumOfRow(matrix->rows[i]) > maxSum) {
                    maxSum = sumOfRow(matrix->rows[i]);
                    max = i;
                }
            }
        }
    ...
    int findMinSumRow(Matrix *matrix) {
        int min = 0;
        int t = findMaxSumRow(matrix);
        int minSum = sumOfRow(matrix->rows[t]);
        for (int i = 0; i < matrix->N; i++) {
            if (matrix->rows[i]) {
                if (sumOfRow(matrix->rows[i]) < minSum) {
                    minSum = sumOfRow(matrix->rows[i]);
                    min = i;
                }
            }
        }
    ...
    MatrixElement *upSort(MatrixElement *head) {
        ...
        while (head != nullptr) {
            ...
            if (new_head == nullptr || node->value < new_head->value) {
                node->next = new_head;
                new_head = node;
            } else {
